...
function getBundleInfo(distPath: string): BundleInfo[] {
  const chunksPath = path.join(distPath, 'static', 'chunks');
  const cssPath = path.join(distPath, 'static', 'css');

  const bundles: BundleInfo[] = [];

  // JSファイルを取得
  if (fs.existsSync(chunksPath)) {
    const jsFiles = fs.readdirSync(chunksPath)
  ...
  if (fs.existsSync(cssPath)) {
    ...
    bundles.push(...cssBundles);
  }

  return bundles.sort((a: BundleInfo, b: BundleInfo) => a.sizeKB - b.sizeKB);
}
